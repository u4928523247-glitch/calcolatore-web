...
let D_manuale = false;  // Traccia se l'utente ha modificato D

// Imposta i campi cliccabili
document.getElementById('A').addEventListener('click', () => setActive('A'));
document.getElementById('orario').addEventListener('click', () => setActive('orario'));
document.getElementById('C').addEventListener('click', () => setActive('C'));
document.getElementById('D').addEventListener('click', function () {
    setActive('D');
    D_manuale = true;  // Segna che D è stato modificato
});
...
function aggiorna() {
    ...
    if (D_manuale && D_val > 0) {
        ...
        // Aggiorna orario se non modificato
        if (orario_val === 0 || document.getElementById('orario').value === '0.00') {
            const orarioAuto = entrata / ore || 0;
            document.getElementById('orario').value = orarioAuto.toFixed(2);
        }
    }
}
